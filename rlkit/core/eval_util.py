...
def get_generic_path_information(paths, stat_prefix=''):
    ...
    if 'normalized_rewards' in paths[0].keys():
        statistics[stat_prefix + 'Average Normalized Returns'] = get_average_normalized_returns(paths)
...
def get_average_normalized_returns(paths):
    returns = [sum(path["normalized_rewards"]) for path in paths]
    return np.mean(returns)
