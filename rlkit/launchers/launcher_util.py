...
try:
    ...
    for code_dir in conf.CODE_DIRS_TO_MOUNT:
        CODE_MOUNTS.append(mount.MountLocal(local_dir=code_dir, pythonpath=True, filter_dir=('data', 'archived_data', 'scripts/plots',
                                                            'scripts/plotting_data', 'scripts/temp_images')))
...
def run_experiment(
    ...
    if mode == 'local':
        ...
        dmode = doodad.mode.Local(skip_wait=skip_wait, gpu_id=gpu_id)
